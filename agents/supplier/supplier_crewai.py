"""Supplier A — Carbon Fiber Specialists — CrewAI-powered agent.
...
Port 6001 · Skills: ``supply:carbon_fiber_panels``, ``supply:carbon_fiber_raw``
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [supplier-a] %(levelname)s  %(message)s",
    datefmt="%H:%M:%S",
)
logger = logging.getLogger("supplier_a")
...
PORT = int(os.environ.get("PORT", SUPPLIER_PORTS["supplier_a"]))
HOST = "0.0.0.0"
...
if CREWAI_AVAILABLE:
    ...
    def check_inventory_tool(part_name: str) -> str:
        """Check the warehouse inventory for a specific automotive part.
        Returns stock quantity, base price, lead time, and certifications.
        Use part identifiers like 'carbon_fiber_panels' or 'carbon_fiber_raw'."""
        part = lookup_part("supplier_a", part_name)
        if part is None:
    ...
    def calculate_pricing_tool(part_name: str, quantity: int) -> str:
        ...
        part_name should be like 'carbon_fiber_panels'. quantity is the number of units."""
...
def _build_rfq_crew(
    ...
    inventory_checker = Agent(
        ...
        backstory=(
            "You are a meticulous warehouse inventory specialist at Carbon Fiber "
            "Specialists GmbH, a leading European supplier of carbon fiber "
            "components for the automotive industry.  Your expertise ensures "
            "accurate stock reporting and reliable supply commitments."
        ),
    ...
    pricing_analyst = Agent(
        ...
        backstory=(
            "You are a senior pricing analyst at Carbon Fiber Specialists GmbH "
            "specialising in B2B automotive parts pricing.  You consider volume "
            "discounts, urgency premiums, and competitive positioning to generate "
            "optimal quotes that win business while protecting margins."
        ),
...
def _build_counter_crew(
    ...
    pricing_analyst = Agent(
        ...
        backstory=(
            "You are the senior pricing analyst at Carbon Fiber Specialists "
            f"GmbH.  For part '{part_name}', our absolute minimum floor price "
...
def _deterministic_counter_eval(
    ...
    result = evaluate_counter_offer("supplier_a", part_name, target_price)
    part = lookup_part("supplier_a", part_name)
...
AGENT_FACTS = AgentFacts(
    ...
    label="Supplier A",
    description=(
        "European carbon fiber components supplier powered by CrewAI. "
        "Specialises in aerospace-grade CFRP body panels and raw carbon "
        "fiber sheets for high-performance automotive applications.  "
    ...
    framework="crewai",
    jurisdiction="EU",
    provider="Carbon Fiber Specialists GmbH",
    skills=[
        Skill(
            id="supply:carbon_fiber_panels",
            description=(
                "High-strength carbon fiber body panels (CFRP), aerospace-grade"
            ),
        ...
        Skill(
            id="supply:carbon_fiber_raw",
            description=(
                "Raw carbon fiber composite sheets for custom fabrication"
            ),
            input_modes=["application/json"],
            output_modes=["application/json"],
    ...
    evaluations=[
        Evaluation(evaluator="self", score=0.92, metric="reliability"),
        Evaluation(
    ...
    policies=[
        Policy(
            ...
            value={"carbon_fiber_panels": 4, "carbon_fiber_raw": 10},
        ),
        Policy(
            name="floor_price_policy",
            description="Minimum 80% of base price on negotiations",
            value=0.80,
        ),
        Policy(
            name="payment_terms",
...
async def lifespan(application: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("Supplier A (CrewAI) starting on port %d …", PORT)
    await _register_with_index()
    await _emit_startup_event()
    logger.info(
        "Supplier A ready at %s  (CrewAI: %s)",
        BASE_URL,
        "enabled" if CREWAI_AVAILABLE else "fallback-only",
    )
    yield
    logger.info("Supplier A shutting down.")


app = FastAPI(
    title="Supplier A — Carbon Fiber Specialists (CrewAI)",
    description=(
        "CrewAI-powered carbon fiber supplier agent with inventory check "
...
async def receive_rfq(envelope: Envelope):
    ...
    # --- Check catalogue ---
    part_info = lookup_part("supplier_a", part_name)
...
async def receive_counter_offer(envelope: Envelope):
    ...
    (80 % of base).  Accepts if target ≥ floor; rejects otherwise.
    """
    payload = envelope.payload
...
async def health():
    ...
    return {
        ...
        "agent_id": AGENT_ID,
        "crewai_available": CREWAI_AVAILABLE,
        "catalog_parts": list(SUPPLIER_A_CATALOG.keys()),
