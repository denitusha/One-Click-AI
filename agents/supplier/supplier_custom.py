"""Supplier B — Precision Metals & Ceramics — Custom Python (rule-based) agent.
...
Port 6002 · Skills: ``supply:titanium_alloy``, ``supply:titanium_fasteners``,
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [supplier-b] %(levelname)s  %(message)s",
    datefmt="%H:%M:%S",
)
logger = logging.getLogger("supplier_b")

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
AGENT_ID = "supplier-b"
AGENT_NAME = "Precision Metals & Ceramics (Custom Python)"
PORT = int(os.environ.get("PORT", SUPPLIER_PORTS["supplier_b"]))
HOST = "0.0.0.0"
...
def _generate_quote(part_name: str, quantity: int) -> dict[str, Any] | None:
    ...
    Rules:
    1. Look up the part in Supplier B's catalogue.
    ...
    """
    part = lookup_part("supplier_b", part_name)
    if part is None:
        return None
...
def _evaluate_counter(
    ...
    """
    result = evaluate_counter_offer("supplier_b", part_name, target_price)
    part = lookup_part("supplier_b", part_name)
...
AGENT_FACTS = AgentFacts(
    ...
    label="Supplier B",
    description=(
        "European precision metals and ceramics supplier using pure rule-based "
        "Python logic (no LLM framework). Specialises in titanium alloys, "
        "titanium fasteners, and carbon-ceramic brake components for "
        "high-performance automotive applications. Deterministic pricing, "
        "fast response times, and strict floor-price negotiation policies."
    ),
    version="1.0.0",
    framework="custom",
    jurisdiction="EU",
    provider="Precision Metals & Ceramics GmbH",
    skills=[
        Skill(
            id="supply:titanium_alloy",
            description=(
                "Ti-6Al-4V double wishbone suspension arms, CNC machined"
            ),
        ...
        Skill(
            id="supply:titanium_fasteners",
            description=(
                "High-strength titanium hex bolts for structural applications"
            ),
            input_modes=["application/json"],
            output_modes=["application/json"],
        ...
        Skill(
            id="supply:ceramic_brake_calipers",
            description=(
                "Carbon-ceramic composite brake calipers, 6-piston, track-ready"
            ),
            input_modes=["application/json"],
            output_modes=["application/json"],
            supported_regions=["EU"],
    ...
    evaluations=[
        Evaluation(evaluator="self", score=0.95, metric="reliability"),
        Evaluation(
    ...
    policies=[
        Policy(
            ...
            value={
                "titanium_alloy": 2,
                "titanium_fasteners": 50,
                "ceramic_brake_calipers": 2,
            },
        ),
        Policy(
            name="floor_price_policy",
            description=(
                "Floor prices vary by part: titanium alloy 85%, "
                "fasteners 90%, ceramic brakes 82% of base price"
            ),
            value={
                "titanium_alloy": 0.85,
                "titanium_fasteners": 0.90,
                "ceramic_brake_calipers": 0.82,
            },
        ),
        Policy(
            name="payment_terms",
...
async def lifespan(application: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("Supplier B (Custom Python) starting on port %d …", PORT)
    await _register_with_index()
    await _emit_startup_event()
    logger.info("Supplier B ready at %s  (pure rule-based, no LLM)", BASE_URL)
    yield
    logger.info("Supplier B shutting down.")


app = FastAPI(
    title="Supplier B — Precision Metals & Ceramics (Custom Python)",
    description=(
...
async def receive_rfq(envelope: Envelope):
    ...
    # --- Check catalogue ---
    part_info = lookup_part("supplier_b", part_name)
...
async def receive_order(envelope: Envelope):
    ...
    part_info = lookup_part("supplier_b", part)
...
async def health():
    ...
    return {
        ...
        "framework": "custom",
        "agent_id": AGENT_ID,
        "catalog_parts": list(SUPPLIER_B_CATALOG.keys()),
