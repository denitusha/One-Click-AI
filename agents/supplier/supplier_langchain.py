"""Supplier C — Powertrain Components — LangChain-powered agent.
...
Port 6003 · Skills: ``supply:aluminum_engine_block``, ``supply:turbocharger_assembly``
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [supplier-c] %(levelname)s  %(message)s",
    datefmt="%H:%M:%S",
)
logger = logging.getLogger("supplier_c")

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
AGENT_ID = "supplier-c"
AGENT_NAME = "Powertrain Components (LangChain)"
PORT = int(os.environ.get("PORT", SUPPLIER_PORTS["supplier_c"]))
HOST = "0.0.0.0"
    ...
    rfq_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                (
                    "You are a pricing analyst at Powertrain Components Srl, "
                    "a European supplier of high-performance powertrain parts "
                    "for the automotive industry, based in Milan, Italy.\n\n"
...
def _deterministic_quote(part_name: str, quantity: int) -> dict[str, Any] | None:
    ...
    part = lookup_part("supplier_c", part_name)
    if part is None:
        return None
...
def _deterministic_counter_eval(
    ...
    result = evaluate_counter_offer("supplier_c", part_name, target_price)
    part = lookup_part("supplier_c", part_name)
...
AGENT_FACTS = AgentFacts(
    ...
    label="Supplier C",
    description=(
        "European powertrain components supplier powered by LangChain. "
        "Specialises in aluminum engine blocks and high-performance "
        "turbocharger assemblies for automotive applications. Uses "
        "LangChain prompt-template → ChatOpenAI → output-parser chains "
        "for intelligent quote generation and counter-offer evaluation."
    ),
    version="1.0.0",
    framework="langchain",
    jurisdiction="EU",
    provider="Powertrain Components Srl",
    skills=[
        Skill(
        ...
        Skill(
            id="supply:turbocharger_assembly",
            description=(
                "High-performance twin-scroll turbocharger with Inconel turbine wheel"
            ),
            input_modes=["application/json"],
            output_modes=["application/json"],
    ...
    certifications=[
        Certification(name="ISO 9001", issuer="TÜV Italia"),
        Certification(name="IATF 16949", issuer="TÜV Italia"),
    ],
    policies=[
        Policy(
            name="min_order_qty",
            ...
            value={
                ...
                "turbocharger_assembly": 1,
            },
        ),
        Policy(
            name="floor_price_policy",
            description=(
                "Floor prices: engine blocks 85% of base, "
                "turbochargers 80% of base price"
            ),
            value={
                "aluminum_engine_block": 0.85,
                "turbocharger_assembly": 0.80,
            },
        ),
        Policy(
            name="payment_terms",
...
async def lifespan(application: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("Supplier C (LangChain) starting on port %d …", PORT)
    _init_chains()
    await _register_with_index()
    await _emit_startup_event()
    logger.info(
        "Supplier C ready at %s  (LangChain: %s)",
        BASE_URL,
        "enabled" if LANGCHAIN_AVAILABLE and _llm is not None else "fallback-only",
    )
    yield
    logger.info("Supplier C shutting down.")


app = FastAPI(
    title="Supplier C — Powertrain Components (LangChain)",
    description=(
        "LangChain-powered powertrain components supplier agent with "
...
async def receive_rfq(envelope: Envelope):
    ...
    # --- Check catalogue ---
    part_info = lookup_part("supplier_c", part_name)
...
async def receive_counter_offer(envelope: Envelope):
    ...
    part_info = lookup_part("supplier_c", part_name) if part_name else None
...
async def receive_order(envelope: Envelope):
    ...
    part_info = lookup_part("supplier_c", part)
...
async def health():
    ...
    return {
        ...
        "catalog_parts": list(SUPPLIER_C_CATALOG.keys()),
